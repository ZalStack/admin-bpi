...
<div class="form-page">
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">Atur judul tampilan, urutan posisi, dan status aktif untuk section <strong>{{ ucfirst($item->section) }}</strong></p>
        </div>
        ...
        <form action="{{ route('admin.beranda.update', $item->id) }}" method="POST"
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    <x-trans-input field="judul" label="Judul Section" :kode="$bahasa->kode" :required="$bahasa->is_default" :item="$item" placeholder="Judul section dalam bahasa {{ $bahasa->nama }}"/>
                </x-lang-panel>
            @endforeach
