...
<div>
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">Kelola judul, urutan tampil, dan status aktif section halaman beranda</p>
    ...
    @if($items->isEmpty())
        <div class="empty-state">
            ...
            <h3 class="empty-title">Belum ada data section beranda</h3>
            <p class="empty-desc">Data section beranda belum diinisialisasi.</p>
        </div>
    @else
                    ...
                    <tbody class="tbody">
                        @foreach($items as $item)
                            <tr class="tr-hover">
                                ...
                                <td class="td text-center">
                                    <button onclick="toggleStatus('beranda', {{ $item->id }})" class="{{ $item->status ? 'badge-active' : 'badge-inactive' }} transition-transform hover:scale-105 cursor-pointer" title="Klik untuk mengubah status aktif/nonaktif">
                                ...
                                <td class="td text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('admin.beranda.edit', $item->id) }}" class="icon-btn-edit" title="Edit Judul & Urutan">
                                            <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.beranda.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                            @csrf
                                            @method('DELETE')
