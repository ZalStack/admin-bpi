...
<div class="form-page">
    <div class="page-header">
        <div>
            ...
            <h1 class="page-title">Add Roadmap</h1>
            <p class="page-subtitle">Add a 4-year roadmap for the Indonesian film ecosystem</p>
        </div>
    ...
    <div class="form-card">
        <form action="{{ route('admin.program-roadmap.store') }}" method="POST" enctype="multipart/form-data"
            x-data="{
            ...
            <div class="input-group">
                <div>
                    ...
                    <input type="text" name="tahun" id="tahun" value="{{ old('tahun', date('Y')) }}" class="form-input" placeholder="e.g.: 2025 or 2025 - 2026" required>
                    @error('tahun')
                ...
                <div>
                    <x-icon-picker name="icon" :value="old('icon', 'fa-solid fa-puzzle-piece')" label="Roadmap Icon" />
                </div>
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="space-y-5">
                    <x-trans-input field="judul" label="Roadmap Theme / Title" :kode="$bahasa->kode" :required="$bahasa->is_default" placeholder="e.g.: Foundation & Consolidation"/>

                    <div>
                        <x-trans-textarea field="deskripsi" label="Roadmap Description" :kode="$bahasa->kode" :required="$bahasa->is_default" rows="3" placeholder="Description of roadmap focus for year {{ $bahasa->nama }}"/>
                    </div>
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <label class="text-xs font-bold uppercase tracking-wider text-gray-700">
                                    Achievement Points (Bullet Points) - Bahasa {{ $bahasa->nama }}
                                </label>
            ...
            <div>
                ...
                <p class="mt-1.5 text-xs text-gray-400">Format: JPG, PNG, WEBP, SVG. Max 2MB. If uploaded, the image will be prioritized on the landing page over the Font Awesome icon.</p>
                @error('gambar')
                    <p class="form-error">{{ $message }}</p>
                @enderror
            </div>
