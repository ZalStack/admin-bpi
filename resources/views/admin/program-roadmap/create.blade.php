...
@section('title', 'Tambah Peta Jalan (Roadmap)')
    ...
    <div class="page-header">
        <div>
            <nav class="breadcrumb">
                ...
                <span>Tambah Peta Jalan</span>
            </nav>
            <h1 class="page-title">Tambah Peta Jalan (Roadmap)</h1>
            <p class="page-subtitle">Tambahkan peta jalan 4 tahun ke depan ekosistem perfilman Indonesia</p>
        </div>
        ...
        <form action="{{ route('admin.program-roadmap.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="input-group">
                <div>
                    ...
                    <input type="text" name="tahun" id="tahun" value="{{ old('tahun', date('Y')) }}" class="form-input" placeholder="cth: 2025 atau 2025 - 2026" required>
                    @error('tahun')
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="space-y-5">
                    <x-trans-input field="judul" label="Tema / Judul Peta Jalan" :kode="$bahasa->kode" :required="$bahasa->is_default" placeholder="cth: Fondasi & Konsolidasi"/>

                    <div>
                        <x-trans-textarea field="deskripsi" label="Deskripsi Peta Jalan" :kode="$bahasa->kode" :required="$bahasa->is_default" rows="3" placeholder="Deskripsi fokus peta jalan tahun {{ $bahasa->nama }}"/>
                    </div>
            ...
            <div>
                ...
                <p class="mt-1.5 text-xs text-gray-400">Format: JPG, PNG, WEBP, SVG. Maksimal 2MB. Jika diunggah, gambar akan diprioritaskan di landing page dibanding icon Font Awesome.</p>
                @error('gambar')
                    <p class="form-error">{{ $message }}</p>
                @enderror
            </div>
