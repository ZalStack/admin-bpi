...
<div class="form-page">
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">Set content and configuration for the section <strong>{{ strtoupper($item->section) }}</strong></p>
        </div>
        ...
        <form action="{{ route('admin.tentang.update', $item->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    @if($item->section === 'intro')
                        <!-- Intro has both Section Title (Badge) and Subtitle (Headline) -->
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <x-trans-input 
                                field="subjudul" 
                                label="Subtitle / Main Headline" 
                                :kode="$bahasa->kode" 
                                :required="$bahasa->is_default" 
                                :item="$item" 
                                placeholder="e.g.: Building the Future of National Cinema"
                            />
                        </div>
                        <div class="mt-2">
                            <x-trans-textarea 
                                ...
                                rows="4" 
                                :item="$item" 
                                placeholder="Introduction description in language {{ $bahasa->nama }}"
                            />
                        </div>
                    @elseif($item->section === 'visi')
                        <div>
                            <x-trans-input 
                                field="judul" 
                                label="Judul Visi" 
                                ...
                                :required="$bahasa->is_default" 
                                :item="$item" 
                                placeholder="e.g.: Visi Kami"
                            />
                        </div>
                        <div class="mt-2">
                            <x-trans-textarea 
                                field="deskripsi" 
                                label="Teks Visi (Deskripsi)" 
                                :kode="$bahasa->kode" 
                                :required="$bahasa->is_default" 
                                rows="3" 
                                :item="$item" 
                                placeholder="Teks visi lengkap dalam bahasa {{ $bahasa->nama }}"
                            />
                        </div>
                        <div class="mt-2">
                            <x-trans-input 
                                field="highlight_words" 
                                label="Kata/Frasa yang Di-highlight" 
                                :kode="$bahasa->kode" 
                                :item="$item" 
                                placeholder="Contoh: unggul dan kompetitif, kompeten, sehat dan kondusif, sehat"
                            />
                            <p class="mt-1 text-xs text-gray-500">
                                Masukkan kata atau frasa yang ingin diberi warna highlight emas pada teks visi di landing page. Pisahkan setiap kata/frasa dengan tanda koma (,).
                            </p>
                        </div>
                    @elseif($item->section === 'misi')
                        <div>
                            <x-trans-input 
                                field="judul" 
                                label="Judul Misi" 
                        ...
                        <div class="mt-2">
                            <x-trans-textarea 
                                field="deskripsi" 
                                label="Deskripsi Pengantar Misi" 
                                :kode="$bahasa->kode" 
                                :required="$bahasa->is_default" 
                                rows="3" 
                                :item="$item" 
                                placeholder="Deskripsi pengantar misi dalam bahasa {{ $bahasa->nama }}"
                            />
                        </div>
                    @endif
                </x-lang-panel>
            ...
            @if(in_array($item->section, ['intro']))
                ...
                <div>
                    ...
                    <p class="mt-1.5 text-xs text-gray-400">Select a new image if you want to change it (Format: JPG, PNG, WEBP max 2MB).</p>
                    @error('gambar')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                        <div>
                            <h3 class="text-base sm:text-lg font-bold text-[#132C5C] flex items-center gap-2">
                                ...
                                Mission Card Points (Poin Card Misi)
                            </h3>
                            <p class="text-xs text-gray-500 mt-0.5">
                                Manage mission cards displayed in the section MISI on the landing page.
                            </p>
                        </div>
                        <button type="button" 
                            @click="
                                        ...
                                        <div x-show="lang === '{{ $b->kode }}'" class="space-y-3">
                                            ...
                                            <div>
                                                ...
                                                <textarea 
                                                    ...
                                                    placeholder="Point description {{ $b->nama }}"></textarea>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
