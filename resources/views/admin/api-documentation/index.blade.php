...
<div x-data="apiDocs()" class="space-y-6">
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title font-poppins">API Documentation</h1>
            <p class="page-subtitle">Complete REST API reference for BPI Admin Panel — endpoint, method, request &amp; response format.</p>
        </div>
        <div class="flex flex-wrap items-center gap-2 shrink-0">
            <span class="count-chip">v1</span>
            <span class="count-chip">{{ $modules->sum(fn ($m) => count($m['endpoints'])) }} Endpoint</span>
            <span class="count-chip">{{ $modules->count() }} Module</span>
        </div>
    </div>
    ...
    <div class="card p-5 md:p-6 relative overflow-hidden">
        ...
        <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            @foreach([
                ['label' => 'Data Format', 'value' => 'JSON', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/>'],
                ...
                ['label' => 'Authentication', 'value' => 'Session (Web Guard)', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>'],
    ...
    <div>
        ...
        <div class="mb-4 rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-xs leading-relaxed text-gray-600">
            <p class="font-semibold text-[#2B4E94]">Multilingual (data-driven):</p>
            <p class="mt-1">All content text is no longer stored per column (<code>judul_id</code>, <code>judul_en</code>). Use the <code>translations</code> object keyed by registered language code from the <code>bahasa</code> table. New languages can be registered via the <code>POST /bahasa</code> endpoint without schema changes.</p>
        </div>
        <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
            <div class="rounded-xl border border-emerald-100 bg-emerald-50/60 p-3.5">
                <p class="font-mono text-xs font-bold text-emerald-700">GET — public</p>
                <p class="mt-1 text-xs leading-relaxed text-gray-600">Open to frontend (throttled to 120 req/min per IP).</p>
            </div>
            ...
            <div class="rounded-xl border border-violet-100 bg-violet-50/60 p-3.5">
                ...
                <p class="mt-1 text-xs leading-relaxed text-gray-600"><code>POST /kontak-form</code> (honeypot + throttle 5/min) &amp; <code>POST /bahasa/switch/&#123;locale&#125;</code>.</p>
            </div>
        </div>
            ...
            <div class="card overflow-hidden">
                <div class="flex items-center justify-between gap-3 border-b border-blue-100 bg-blue-50/60 px-5 py-3.5">
                    <div class="flex items-center gap-2.5">
                        ...
                        <p class="text-sm font-semibold text-blue-800">Resource Created</p>
                    </div>
            ...
            <div class="card overflow-hidden">
                <div class="flex items-center justify-between gap-3 border-b border-amber-100 bg-amber-50/60 px-5 py-3.5">
                    <div class="flex items-center gap-2.5">
                        ...
                        <p class="text-sm font-semibold text-amber-800">Validation Failed</p>
                    </div>
            ...
            <div class="card overflow-hidden">
                <div class="flex items-center justify-between gap-3 border-b border-rose-100 bg-rose-50/60 px-5 py-3.5">
                    <div class="flex items-center gap-2.5">
                        ...
                        <p class="text-sm font-semibold text-rose-800">Data Not Found</p>
                    </div>
  ...
  "message": "Resource not found"
    ...
    <div class="card p-5 md:p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            @foreach([
                ['code' => '200', 'title' => 'OK', 'desc' => 'Request processed successfully', 'style' => 'text-emerald-700 bg-emerald-50 ring-emerald-200'],
                ['code' => '201', 'title' => 'Created', 'desc' => 'Resource created successfully', 'style' => 'text-blue-700 bg-blue-50 ring-blue-200'],
    ...
    <div class="card overflow-hidden">
        ...
        <pre x-ref="curlExample" class="overflow-x-auto bg-gradient-to-br from-gray-900 to-[#132C5C] p-5 text-[0.78rem] leading-relaxed font-mono text-gray-200"><code># GET - Fetch all banners
curl -X GET "{{ url('api/admin/v1') }}/banner" \
  -H "Accept: application/json"

# POST - Create a new banner (text per language inside "translations")
    ...
    function apiDocs() {
        ...
        return {
            ...
            async copyJson(refEl, event) {
                ...
                if (event && event.currentTarget && typeof event.currentTarget.blur === 'function') event.currentTarget.blur();
                this.showToast('JSON copied successfully');
            },
        };
    }
</script>
