    ...
    <div class="page-header">
        <div>
            <nav class="breadcrumb">
                ...
                <a href="{{ route('admin.proyek.galeri.index', $proyek->id) }}">Gallery</a>
                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                </svg>
                <span>Edit</span>
            ...
            <p class="page-subtitle">Project: {{ $proyek->translateField('judul') }}</p>
        </div>
        <a href="{{ route('admin.proyek.galeri.index', $proyek->id) }}" class="btn-outline">Back</a>
    </div>

    <div class="form-card">
        <form action="{{ route('admin.proyek.galeri.update', [$proyek->id, $galeri->id]) }}" method="POST" enctype="multipart/form-data"
                ...
                @if($galeri->gambar)
                    <div class="mb-3" x-data="{ deleting: false }">
                        ...
                        <div class="flex items-start gap-3">
                            <img id="current-gambar" src="{{ asset('storage/proyek/galeri/'.$galeri->gambar) }}" alt="galeri" class="h-44 w-full max-w-md rounded-xl object-cover ring-1 ring-gray-200">
                            <button type="button" @click="if(!confirm('Are you sure you want to delete this image?')) return; deleting=true; fetch('{{ route('admin.image.delete') }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'},body:JSON.stringify({model:'ProyekGaleri',id:{{ $galeri->id }},field:'gambar'})}).then(r=>r.json()).then(d=>{if(d.success){document.getElementById('current-gambar').style.display='none';this.style.display='none';}else{alert(d.message);deleting=false;}}).catch(()=>{alert('An error occurred.');deleting=false;})" class="shrink-0 mt-2 inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 transition-colors" :disabled="deleting">
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    ...
                    <x-trans-input field="judul" label="Title" :kode="$bahasa->kode" :item="$galeri"/>
                    <x-trans-textarea field="deskripsi" label="Description" :kode="$bahasa->kode" rows="3" :item="$galeri"/>
                </x-lang-panel>
            @endforeach
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('admin.proyek.galeri.index', $proyek->id) }}" class="btn-outline">Cancel</a>
            </div>
        </form>
    </div>
</div>
