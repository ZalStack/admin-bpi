...
<div class="form-page">
    ...
    @php
        $initialSocialMedia = $item->social_media->map(function ($s) {
            return [
                'id' => $s->id,
                'platform' => $s->platform,
                'username' => $s->username,
        ...
        $initialEmails = $item->email->map(function ($e) {
            return [
                'id' => $e->id,
                'email' => $e->email,
        ...
        $initialPhones = $item->phone->map(function ($p) {
            return [
                'id' => $p->id,
                'number' => $p->number,
    ...
    <div class="form-card" x-data="{
        ...
        socialMedia: @js($initialSocialMedia),
        emails: @js($initialEmails),
        phones: @js($initialPhones),
        gmapsUrl: @js(old('gmaps_url', $item->gmaps_url ?? '')),
        latitude: @js(old('latitude', $item->latitude ?? '')),
        longitude: @js(old('longitude', $item->longitude ?? '')),
        detectedCoords: '',
        ...
        <form action="{{ route('admin.kontak.update', $item->id) }}" method="POST">
            ...
            @foreach ($bahasas as $bahasa)
                ...
                <x-lang-panel :kode="$bahasa->kode" class="space-y-4">
                    <x-trans-input field="judul" label="Judul Halaman Kontak" :kode="$bahasa->kode" :required="$req" :item="$item" placeholder="e.g.: Hubungi Kami / Contact Us"/>
                    <x-trans-input field="nama_kantor" label="Nama Kantor / Sekretariat" :kode="$bahasa->kode" :required="false" :item="$item" placeholder="e.g.: Sekretariat Pusat BPI"/>
                    <x-trans-textarea field="alamat" label="Alamat Kantor Lengkap" :rows="3" :kode="$bahasa->kode" :required="false" :item="$item" placeholder="e.g.: Gedung Film lt. 2, Jl. MT Haryono Kav. 47-48, Cikoko, Pancoran, Jakarta Selatan 12770"/>
                    <x-trans-input field="jam_operasional" label="Jam Operasional" :kode="$bahasa->kode" :required="false" :item="$item" placeholder="e.g.: Senin – Jumat: 09.00 – 17.00 WIB"/>
                </x-lang-panel>
            @endforeach
