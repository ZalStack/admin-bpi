        ...
        <form action="{{ route('admin.struktur.store') }}" method="POST" enctype="multipart/form-data"
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    ...
                    <x-trans-input field="departemen" label="Department / Unit" :kode="$bahasa->kode" placeholder="Department / Unit in {{ $bahasa->nama }} (e.g. {{ $bahasa->kode === 'en' ? 'Organization Department' : 'Bidang Organisasi' }})" list="departemen-options-{{ $bahasa->kode }}"/>
