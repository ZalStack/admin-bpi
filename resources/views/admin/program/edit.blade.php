        ...
        <form action="{{ route('admin.program.update', $item->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="input-group">
                <div>
                    ...
                    <input type="text" name="icon" id="icon" value="{{ old('icon', $item->icon) }}" class="form-input" placeholder="fa-solid fa-coins">
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    <x-trans-input field="judul" label="Judul Program" :kode="$bahasa->kode" :required="$bahasa->is_default" :item="$item" placeholder="cth: Pembiayaan & Investasi"/>
                    <div class="mt-2">
                        <x-trans-textarea field="deskripsi" label="Deskripsi Program" :kode="$bahasa->kode" :required="$bahasa->is_default" :item="$item" rows="4" placeholder="Deskripsi pilar program dalam bahasa {{ $bahasa->nama }}"/>
                    </div>
                </x-lang-panel>
            @endforeach
            ...
            <div>
                ...
                @if($item->gambar)
                    <div class="mb-3">
                        ...
                        <img src="{{ asset('storage/program/'.$item->gambar) }}" alt="program" class="h-44 w-full max-w-md rounded-xl object-cover ring-1 ring-gray-200">
                    </div>
                @endif
