    ...
    <div class="form-card">
        <form action="{{ route('admin.program.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="input-group">
                <div>
                    <x-icon-picker name="icon" :value="old('icon', 'fa-solid fa-coins')" label="Icon Pilar Program" />
                </div>
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    <x-trans-input field="judul" label="Judul Pilar Program" :kode="$bahasa->kode" :required="$bahasa->is_default" placeholder="cth: Pembiayaan dan Investasi"/>
                    <x-trans-textarea field="deskripsi" label="Deskripsi Pengantar" :kode="$bahasa->kode" :required="$bahasa->is_default" rows="3" placeholder="Deskripsi ringkas mengenai pilar program ini..."/>
                </x-lang-panel>
            @endforeach
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Tambahkan kartu-kartu sub-poin yang ada di dalam pilar program ini.</p>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div x-show="poinList.length === 0" class="text-center py-6 border-2 border-dashed border-gray-200 rounded-2xl bg-white">
                        <p class="text-xs text-gray-400">Belum ada sub-poin ditambahkan. Klik "Tambah Sub-Poin" di atas.</p>
                    </div>
                </div>
            </div>
