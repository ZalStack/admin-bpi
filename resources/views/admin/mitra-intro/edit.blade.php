...
@section('title', 'Partner Page Intro')
...
<div class="form-page">
    ...
    <div class="form-card">
        <form action="{{ route('admin.mitra-intro.update', $item->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @foreach ($bahasas as $bahasa)
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <x-trans-input 
                            field="subjudul" 
                            label="Subheadline / Main Headline" 
                            :kode="$bahasa->kode" 
                            :required="$bahasa->is_default" 
                            :item="$item"
                    ...
                    <div class="mt-2">
                        <x-trans-textarea 
                            ...
                            rows="4" 
                            :item="$item"
                            placeholder="Partnership description in language {{ $bahasa->nama }}"
                        />
                    </div>
                </x-lang-panel>
            @endforeach
            ...
            <div>
                <label for="gambar" class="form-label">Intro Photo / Image</label>
                @if($item->gambar)
