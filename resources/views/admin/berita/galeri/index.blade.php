...
<div x-data="{
    ...
    openPreview(image, judul, deskripsi, urutan) {
        ...
        this.activeJudul = judul || 'Photo Gallery';
    ...
    <div class="page-header">
        <div>
            ...
            <h1 class="page-title">News Gallery</h1>
            <p class="page-subtitle">News: <span class="font-semibold text-gray-700">{{ $berita->translateField('judul') }}</span></p>
    ...
    @if($items->isEmpty())
        <div class="empty-state">
            ...
            <h3 class="empty-title">No gallery for this news yet</h3>
            <p class="empty-desc">Add supporting gallery images for this news article.</p>
        </div>
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            @foreach($items as $galeri)
                @php
                    $judulGaleri = $galeri->translateField('judul') ?? $galeri->translateField('caption') ?? 'Documentation Photo';
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group flex flex-col justify-between"
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-3.5">
                            <div class="flex justify-between items-start">
                                <span class="flex h-6 px-2 items-center justify-center rounded-lg bg-black/50 text-white text-xs font-bold backdrop-blur-sm">#{{ $galeri->urutan }}</span>
                                 <span class="flex h-7 w-7 items-center justify-center rounded-full bg-white/30 text-white backdrop-blur-sm hover:bg-white hover:text-gray-900 transition-colors" title="Click to enlarge">
                    ...
                    <div class="p-4 flex-1 flex flex-col justify-between">
                        <div>
                            ...
                            @else
                                <p class="text-xs text-gray-400 font-poppins mt-1 italic">No description</p>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-3.5 pt-3 border-t border-gray-100">
                            ...
                            <div class="flex items-center gap-1">
                                <a href="{{ route('admin.berita.galeri.edit', [$berita->id, $galeri->id]) }}" class="icon-btn-edit !h-8 !w-8" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form action="{{ route('admin.berita.galeri.destroy', [$berita->id, $galeri->id]) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this gallery photo?')">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="relative w-full max-w-3xl bg-white rounded-3xl overflow-hidden shadow-2xl border border-white/20"
            ...
            <div class="relative max-h-[60vh] bg-slate-950 flex items-center justify-center overflow-hidden">
                ...
                <div class="absolute bottom-3 left-4">
                    <span class="px-2.5 py-1 rounded-lg bg-black/60 text-white text-xs font-semibold backdrop-blur-sm" x-text="`Order #${activeUrutan}`"></span>
                </div>
            </div>
            ...
            <div class="p-6 sm:p-7 bg-white">
                ...
                <p class="mt-2 text-sm text-gray-600 leading-relaxed font-poppins" x-text="activeDeskripsi || 'No additional description for this gallery photo.'"></p>
                <div class="mt-5 flex justify-end">
                    <button type="button" @click="showModal = false" class="btn-outline text-xs py-2 px-4">Close Preview</button>
                </div>
            </div>
        </div>
    </div>
