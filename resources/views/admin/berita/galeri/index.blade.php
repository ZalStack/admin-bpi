...
<div>
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">{{ $berita->judul_id }}</p>
            ...
            @foreach($galeris as $galeri)
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group">
                    <div class="relative h-48 overflow-hidden">
                        @if($galeri->gambar)
                            <img src="{{ asset('storage/berita/galeri/'.$galeri->gambar) }}" alt="{{ $galeri->caption_id }}" class="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-110" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                        @else
                    ...
                    <div class="p-4">
                        <h3 class="font-semibold text-gray-800 font-poppins">{{ Str::limit($galeri->caption_id ?? 'Galeri', 25) }}</h3>
                        <p class="text-sm text-gray-500 font-poppins mt-1">{{ Str::limit($galeri->caption_en ?? '', 40) }}</p>
