...
<div class="form-page">
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">Tambahkan berita baru lengkap dengan informasi detail</p>
        </div>
        ...
        <form action="{{ route('admin.berita.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="input-group">
                <div>
                    ...
                    <input type="text" name="penulis" id="penulis" value="{{ old('penulis') }}" class="form-input" placeholder="Nama penulis" required>
                    @error('penulis')
            ...
            @foreach ($bahasas as $bahasa)
                ...
                <x-lang-panel :kode="$bahasa->kode" class="grid grid-cols-1 gap-4">
                    ...
                    <div class="input-group">
                        <x-trans-input field="judul" label="Judul" :kode="$bahasa->kode" :required="$req" placeholder="Judul dalam bahasa {{ $bahasa->nama }}"/>
                        <x-trans-input field="kategori" label="Kategori" :kode="$bahasa->kode" :required="$req" placeholder="{{ $req ? 'cth: Kegiatan' : 'e.g: Activity' }}"/>
                    </div>

                    <x-trans-textarea field="ringkasan" label="Ringkasan" :kode="$bahasa->kode" :required="$req" rows="3" placeholder="Ringkasan dalam bahasa {{ $bahasa->nama }}"/>

                    <div class="mt-4">
                        <x-trans-textarea field="isi" label="Isi Berita" :kode="$bahasa->kode" :required="$req" rows="6" placeholder="Isi lengkap dalam bahasa {{ $bahasa->nama }}"/>
                    </div>

                    <div class="mt-4">
                        <x-trans-textarea field="kutipan" label="Kutipan" :kode="$bahasa->kode" rows="2" placeholder="Kutipan dalam bahasa {{ $bahasa->nama }}"/>
                    </div>
                </x-lang-panel>
            @endforeach
